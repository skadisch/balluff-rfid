...
const connect = ({ ipAddress, port, onConnect, onError }) => {
    ...
    const claimIoLink = ({
        ...
        const writeAcyclicProcessData = async ({ index, subIndex, data }) => {
            if (claimedIoLink !== handle) {
                throw new Error("io link already released");
            }
            ...
            const { statusCode: sc1 } = await conn.writeIoLinkAcylicData({
                index,
                subIndex,
                data
            });
            ...
            if (sc1 !== balluff007.statusCodes.OK) {
                return {
                    error: Error("write io link acyclic data failed", { cause: balluff007.errorFromStatusCode({ statusCode: sc1 }) })
                };
            }
